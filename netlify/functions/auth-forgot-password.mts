...
async function sendEmail(to: string, resetLink: string) {
  ...
  if (!response.ok) {
    console.error("auth_forgot_password:resend_failed", response.status, await response.text());
    return false;
  }
  return true;
}
...
export default async function handler(req: Request) {
  ...
  try {
    ...
    let users = await supabase("admin_users", { query: `select=id,email&email=eq.${encodeURIComponent(email)}&limit=1` });
    if (!users.length) {
      const userId = `env-admin-${hashToken(adminEmail).slice(0, 24)}`;
      await supabase("admin_users", {
        method: "POST",
        prefer: "return=minimal",
        body: [{ id: userId, email: adminEmail, password_hash: "pending", password_salt: "pending", created_at: new Date().toISOString(), updated_at: new Date().toISOString() }],
      });
    ...
    await supabase("admin_password_resets", {
      method: "POST",
      prefer: "return=minimal",
      body: [{ id: randomUUID(), token_hash: hashToken(token), user_id: users[0].id, expires_at: expiresAt, created_at: new Date().toISOString() }],
    });
