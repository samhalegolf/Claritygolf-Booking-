...
async function upsertAdminUser(email: string, password: string) {
  const userId = `env-admin-${hashToken(email).slice(0, 24)}`;
...
async function findAdminUser(email: string) {
  const rows = await supabaseRequest(
    "admin_users",
    { query: `select=id,email,password_hash,password_salt&email=eq.${encodeURIComponent(email)}&limit=1` },
  );
  return rows[0] || null;
}
...
export default async function handler(req: Request) {
  ...
  try {
    ...
    const userId = user?.id || `env-admin-${hashToken(adminEmail).slice(0, 24)}`;
