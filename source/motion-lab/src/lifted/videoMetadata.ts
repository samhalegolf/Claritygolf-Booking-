/**
 * LIFTED FROM THE BOOKING APP.
 *
 * Source: src/modules/video-analysis/utils/videoMetadata.ts
 * Copied: 2026-09-21
 *
 * A ONE-WAY COPY, not a shared module. The lab imports nothing from ../src,
 * and that isolation is worth more than the duplication costs: the booking
 * app's video workspace can change freely without breaking an experiment, and
 * an experiment cannot break a coach's video tools.
 *
 * If a genuine bug is found here, it is worth fixing in BOTH places -- but
 * that is a deliberate act, not something a refactor does by accident.
 */
