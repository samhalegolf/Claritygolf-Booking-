/**
 * The two Vite plugins the pose detector needs, shared by every config that
 * serves it: the standalone lab (vite.lab.config.ts) and the booking app
 * (../vite.config.ts), which mounts the lab inside its video workspace.
 *
 * Both resolve their files relative to THIS file, so it must stay next to
 * the lab's src/ regardless of which config imports it.
 */
...
/** Everything a Vite config needs to serve the pose detector. */
export const motionLabPlugins = (): Plugin[] => [mediapipeWasm(), poseWorker()];
