...
function requestFromEvent(event: any) {
  ...
  const rawUrl =
    event.rawUrl ||
    `https://${host}${event.rawPath || event.path || "/api/auth/change-password"}${event.rawQuery ? `?${event.rawQuery}` : ""}`;
...
export async function handler(event: any, context: Context) {
  return lambdaResponse(await handleBookingApiRoute(requestFromEvent(event), "/api/auth/change-password", context));
}
